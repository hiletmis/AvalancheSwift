...
public final class AvalancheSwift {
    ...
    public func delegateAvax(info: delegatorInfo, amount: String, isValidate: Bool = false, completion: @escaping (_ transaction: [UInt8]?, _ signatureIndexes: [Int])->()) {
    ...
    public func importAvaxC(from: String, to: String, web3Address: String, completion: @escaping (_ transaction: [UInt8]?, _ signatureIndexes: [Int])->()) {
    ...
    public func importAvax(from: String, to: String, completion: @escaping (_ transaction: [UInt8]?, _ signatureIndexes: [Int])->()) {
        guard let from = chainIdentifier.init(rawValue: from) else { return }
        guard let to = chainIdentifier.init(rawValue: to) else { return }
    ...
    public func exportToAvaxC(from: String, to: String, amount: String, web3Address: String, nonce: String, completion: @escaping (_ transaction: [UInt8]?, _ signatureIndexes: [Int])->()) {
        guard let from = chainIdentifier.init(rawValue: from) else { return }
        guard let to = chainIdentifier.init(rawValue: to) else { return }
    ...
    public func exportAvax(from: String, to: String, amount: String, completion: @escaping (_ transaction: [UInt8]?, _ signatureIndexes: [Int])->()) {
        guard let from = chainIdentifier.init(rawValue: from) else { return }
        guard let to = chainIdentifier.init(rawValue: to) else { return }
